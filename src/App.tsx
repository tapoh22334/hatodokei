...
function App() {
  ...
  const handleMuteMasterChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleMuteChildChange = (time: number) => {
    ...
    if (rowChild !== undefined) {
        ...
        invoke('set_table_row', {row: {time: rowChild.time, active: rowChild.active}});
  ...
  const toTimeString = (hhmm: number) => {
    return ('00' + (hhmm / 100 >> 0).toString()).slice(-2) + ':' + ('00' + (hhmm % 100).toString()).slice(-2);
  }

  const CardTimeSwitch: React.VFC<TTElement> = (props) => {
      return (<Card variant="outlined">
      <Stack spacing={1} direction="row" sx={{ mx: 1, my: 0 }} alignItems="center" justifyContent="center">
      <IconButton aria-label="play" onClick={handlePlayClick.bind(null, props.time)}>
      <PlayArrow />
      </IconButton>
      <Box component="span"> {toTimeString(props.time)} </Box>
      <Switch aria-label={`mute-child-switch-${props.time}`}
    ...
    <div className="App">
      ...
      <Toolbar>
        <Box sx={{ width: 220 }} alignItems="center" position="sticky">
          <Stack spacing={0} direction="row" sx={{ my: 0 }} alignItems="center" justifyContent="center">
          <VolumeDown />
          <Slider sx={{color: '#fff'}} aria-label="MasterVolume" value={masterVolume} onChange={handleVolumeChange} />
          <VolumeUp />
          <Switch color="default" aria-label="mute_master" checked={!disabled} onChange={handleMuteMasterChange}/>
          </Stack>
        </Box>
        </Toolbar>
      </AppBar>

        <Stack sx={{ mt: 8 }} spacing={0.3} alignItems="center" justifyContent="center">
          {timeTable.length > 0 &&
            timeTable.map((row) => {
              const labelId = `stack-list-label-${row["time"]}`;
              return (
                 <CardTimeSwitch time={row.time} active={row.active}/>
              );
            })
          }
        </Stack>
